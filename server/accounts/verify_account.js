...
const verifyAccountRequest = (connection) => (req, res) => {
	//handle all outcomes
	...
	return getInformationFromSignups(connection, req)
...
const getInformationFromSignups = (connection, req) => new Promise( async (resolve, reject) => {
	//get the saved data
...
const createAccount = (connection) => (record) => new Promise( async (resolve, reject) => {
	...
	setTimeout(async () => {
		...
		let moveQuery = 'INSERT IGNORE INTO accounts (email, username, salt, hash, promotions) VALUES (?, ?, ?, ?, ?);';
		await connection.query(moveQuery, [record.email, record.username, record.salt, record.hash, record.promotions]);
...
module.exports = {
	...
	//for testing
	getInformationFromSignups: getInformationFromSignups,
