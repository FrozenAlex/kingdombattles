...
const validatePassword = (connection) => (fields) => new Promise( async (resolve, reject) => {
	...
	const accountQuery = 'SELECT id, email, username, salt, hash FROM accounts WHERE email = ?;';
	...
	const newHash = await bcrypt.hash(fields.password, accountRecord.salt);

	//compare the "passwords"
	if (accountRecord.hash != newHash) {
		return reject({msg: 'Incorrect email or password', extra: [fields.email, 'Did not find this password']});
	}
