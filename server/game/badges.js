...
const ownedRequest = async (req, res) => {
	...
	// If not supplied with id use user id
	let id = req.body.id || user.id;
	// get user badges
	let badgesOwned = await getBadgesOwned(id)
...
const rewardBadge = async (id, badgeName) => {
	...
	let packet = (await pool.promise().query(
		'INSERT INTO badges (accountId, name) SELECT ?, ? FROM DUAL WHERE NOT EXISTS(SELECT 1 FROM badges WHERE accountId = ? AND name = ?);', [id, badgeName, id, badgeName]))[0]
...
function runBadgeTicks() {
	...
	//King Of The Hill
	let kingOfTheHillBadgeTickJob = new CronJob('* * * * * *', async () => { //once a minute
		//NOTE: sloppy implementation - people who have the badge may get "rewarded" twice. Thankfully rewardBadge() prevents this.
		let ladderResults = await getLadderData('parameter not used (yet)', 0, 1)
		...
		let contenders = (await pool.promise().query('SELECT * FROM badgesTimespan WHERE name = "King Of The Hill";'))[0]

		const day = 1000 * 60 * 60 * 24; //milliseconds
		const now = new Date();
		const qualifyTime = contenders.length > 0 ? new Date(contenders[0].qualifyTime) : null;

		//if someone qualifies (1 day)
		if (contenders.length > 0 && now - qualifyTime >= day) {
			rewardBadge(contenders[0].accountId, contenders[0].name);
			await pool.promise().query('DELETE FROM badgesTimespan WHERE id = ?;', [contenders[0].id])
			return;
		}

		//if someone is still a contender for this badge
		if (contenders.length > 0 && ladderResults[0].id === contenders[0].accountId) {
			//DO NOTHING
